...
class Config:
    ...
    CANDLE_LIMIT         = 200
    SWING_LOOKBACK       = 5
    MIN_CONFLUENCE_SCORE = 3    # lowered from 4
    MIN_RR_RATIO         = 1.2  # lowered from 1.5
    SL_BUFFER_PCT        = 0.002
    DISPLACEMENT_MULT    = 1.0  # lowered from 1.3
...
class PublicDataFetcher:
    ...
    # Full forex list including CHFJPY and all major pairs
    FOREX_PAIRS = [
...
class SMCAnalysisEngine:
    ...
    def ltf_choch(self, ltf_candles: List[Candle],
        ...
        recent  = ltf_candles[-50:]       # increased from 30
        swings  = self.detect_swings(recent, lb=2)  # reduced lb from 3
    ...
    def analyse(self,
        ...
        # ── Bias (warning only — does NOT block signal) ───
...
async def symbol_handler(update: Update,
    ...
    loading = await update.message.reply_text(
        ...
        f'⏳ Fetching candles + running SMC analysis...',
        parse_mode='HTML'
    )

    try:
        htf_candles    = fetcher.fetch(symbol, Config.HTF, 200)
        ltf_candles    = fetcher.fetch(symbol, Config.LTF, 100)
        daily_candles  = fetcher.fetch(symbol, '1d',        30)
        weekly_candles = fetcher.fetch(symbol, '1w',        20)
